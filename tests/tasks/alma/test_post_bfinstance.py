...
def test_NewInstancetoAlma_post_request(mocker: MockerFixture):
    ...
    mocker.patch(
        "ils_middleware.tasks.alma.post_bfinstance.get_env_vars", new=mock_get_env_vars
    )
...
def test_putInstanceToAlma_success():
...
def test_putInstanceToAlma_failure():
...
def test_putInstanceToAlma_internal_server_error():
...
def test_putInstanceToAlma_unexpected_status_code():
