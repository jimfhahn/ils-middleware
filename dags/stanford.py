...
from sinopia import UpdateIdentifier, map_to_folio
...
with DAG(
    ...
    urls = ["http://sinopia-example.io/stanford/01",
            "http://sinopia-example.io/stanford/02",
            "http://sinopia-example.io/stanford/03"]

    branch_ils = BranchPythonOperator(
        task_id="ils",
        python_callable=choose_ils
    )
    ...
    send_to_symphony = BashOperator(
        task_id="symphony_send",
        bash_command=connect_symphony_cmd
    )
...
sinopia_to_symphony_json >> send_to_symphony >> processed_sinopia
